...
static VkDescriptorSet          g_DescriptorSet2 = VK_NULL_HANDLE;
    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                //printf("%lu\n", (uint32_t)pcmd->TextureId);

                if ((uint64_t)pcmd->TextureId < 500)
                    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, g_PipelineLayout, 0, 1, &g_DescriptorSet, 0, 0);
                else
                    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, g_PipelineLayout, 0, 1, (VkDescriptorSet*)pcmd->TextureId, 0, 0);
...
bool ImGui_ImplVulkan_CreateDeviceObjects()
{
    ...
    // Create Descriptor Set:
    {
        ...
        err = vkAllocateDescriptorSets(v->Device, &alloc_info, &g_DescriptorSet2);
        check_vk_result(err);
    }
